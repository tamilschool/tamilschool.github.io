...
// Helper function to shuffle array (moved outside hook for reuse)
 ...
 * 
 * NOTE: Uses useMemo to ensure pools are only generated once per kural set
 */
export function useQuestionPool(kurals: Thirukkural[]): QuestionPoolState {
  // Memoize the question pool generation - only regenerate when kurals array changes
  // Use kurals.length as a stable dependency (array identity changes on every render)
  return useMemo(() => {
    // Return empty pools if no kurals
    if (kurals.length === 0) {
      return {
        lastWords: [],
        firstWords: [],
  ...
  }, [kurals.length]); // Only regenerate when the number of kurals changes
}
